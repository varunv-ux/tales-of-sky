  ...
  return (
    <>
      ...
      <div className="flex h-screen font-sans text-taupe-800 dark:text-taupe-200 bg-taupe-200 dark:bg-taupe-950 antialiased">
        {/* Mobile header */}
        <div className="fixed top-0 left-0 right-0 z-20 flex items-center justify-between px-5 py-3 bg-taupe-200 dark:bg-taupe-950 border-b border-taupe-300 dark:border-taupe-800 md:hidden">
          ...
          <div className="relative">
            <button onClick={() => setMobileMenuOpen((v) => !v)} aria-label="Actions menu" className="p-1.5 text-taupe-500 dark:text-taupe-400">
            ...
            {mobileMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 z-40 w-48 bg-white dark:bg-taupe-800 rounded-xl shadow-lg border border-taupe-200 dark:border-taupe-700 py-1.5 text-sm">
                  {/* Unit toggle */}
                  <div className="flex items-center justify-between px-4 py-2">
                    <span className="text-taupe-500 dark:text-taupe-400 text-xs font-medium">Unit</span>
                    <div className="flex gap-1">
                      {['C', 'F'].map((u) => (
                        <button
                          key={u}
                          onClick={() => { handleSetUnit(u); setMobileMenuOpen(false); }}
                          className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${
                            unit === u
                              ? 'bg-taupe-800 dark:bg-taupe-200 text-white dark:text-taupe-900'
                              : 'text-taupe-500 dark:text-taupe-400 hover:bg-taupe-100 dark:hover:bg-taupe-700'
                          }`}
                        >
                          °{u}
                        </button>
                  ...
                  <div className="border-t border-taupe-100 dark:border-taupe-700 my-1" />
                  {/* Refresh */}
                  <button
                    onClick={() => { handleRefresh(); setMobileMenuOpen(false); }}
                    className="w-full flex items-center gap-3 px-4 py-2 text-taupe-700 dark:text-taupe-300 hover:bg-taupe-50 dark:hover:bg-taupe-700 text-left"
                  >
                  ...
                  {/* Dark mode */}
                  <button
                    onClick={() => { setDarkMode((d) => !d); setMobileMenuOpen(false); }}
                    className="w-full flex items-center gap-3 px-4 py-2 text-taupe-700 dark:text-taupe-300 hover:bg-taupe-50 dark:hover:bg-taupe-700 text-left"
                  >
                    {darkMode ? (
                  ...
                  {/* Profanity mode */}
                  <button
                    onClick={() => { handleSetProfanityMode(!profanityMode); setMobileMenuOpen(false); }}
                    className="w-full flex items-center gap-3 px-4 py-2 text-taupe-700 dark:text-taupe-300 hover:bg-taupe-50 dark:hover:bg-taupe-700 text-left"
                  >
                    <span className="text-sm">🤬</span>
                    {profanityMode ? 'Profanity OFF' : 'Profanity ON'}
                  </button>
                </div>
              </>
            )}
        ...
        <main className="flex-1 m-1.5 px-5 sm:px-10 pt-16 md:pt-12 pb-12 bg-taupe-50 dark:bg-taupe-900 overflow-y-auto max-h-[calc(100vh-12px)] rounded-[2rem] relative">
          <div className="absolute top-4 right-5 hidden md:flex items-center space-x-2 z-10">
            {/* Refresh button */}
            <button
